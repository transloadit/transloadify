...
class Parser {
    constructor() {
        this._opts = [];
        this._longs = Object.create(null);
        this._shorts = Object.create(null);
    }
    ...
    _parse(args, opts, tgts) {
        if (args.length === 0) return { options: opts, targets: tgts };

        let arg = args.shift();

        if (arg === "--") {
            while (args.length > 0) tgts.push(args.shift());
            return _parse(args, opts, tgts);
        }

        if (arg.startsWith("--")) return _parseLong(arg, args, opts, tgts);
        if (arg !== "-" && arg.startsWith("-")) return _parseShort(arg, args, opts, tgts);
        
        tgts.push(arg);
        return _parse(args, opts, tgts);
    }

    _parseLong(arg, args, opts, tgts) {
        let name, value;
    ...
    _parseShort(arg, args, opts, tgts) {
        let chars = Array.from(arg.slice(1));

        for (let opt = chars.shift(); chars.length > 0; opt = chars.shift()) {
